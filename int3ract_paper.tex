...
function \texttt{JNK\_int()} can be applied to multiplicative
...
\item
  ...
  \texttt{JNK\_int()} requires only a coefficient vector and the
...
\bottomrule\noalign{}
\endlastfoot
\texttt{JNK\_lm()} & \texttt{lm}, \texttt{glm} & \texttt{JNK\_int()} \\
...
\texttt{JNK\_int()} & \emph{any} (generic) & \texttt{parFun()},
\texttt{seFun()} \\
\end{longtable}
...
\textbf{Core frequentist engine (\texttt{JNK\_int()}).}\\
The function \texttt{JNK\_int()} accepts \texttt{name} (character vector
of variable names), \texttt{coefs} (named numeric vector of model
...
Crucially, \texttt{JNK\_int()} is model-agnostic. Any researcher who can
extract a coefficient vector and covariance matrix from their model
(whether fitted by \texttt{lm()}, \texttt{glmer()}, \texttt{coxph()}, or
any other estimator) can call \texttt{JNK\_int()} directly. The function
can also be applied to Bayesian estimations, if one would reduce the
posteriors to their mean and covariance structure.
...
convention before delegating to \texttt{JNK\_int()}.
...
\texttt{lm}/\texttt{glm} object but can also be supplied directly, which
is always necessary for \texttt{JNK\_int()} and \texttt{JNK\_siena()}
...
each effect in the \textbf{RSiena} effects object\footnote{While each
  effect in \textbf{RSiena} is named, within a given model, effects are
  not necessarily identifiable by their name but the same name might
  appear multiple times for different dependent variables or effect
  types}.
...
\texttt{JNK\_int()}}{Direct Use of JNK\_int()}}\label{direct-use-of-jnk_int}

Because \texttt{JNK\_int()} requires only a coefficient vector and a
...
\FunctionTok{JNK\_int}\NormalTok{(}\AttributeTok{name  =} \FunctionTok{c}\NormalTok{(}\StringTok{"x"}\NormalTok{, }\StringTok{"z"}\NormalTok{),}
...
Snijders, Koskinen, and Ripley 2026; Koskinen and Snijders 2023), a
series of conditional posterior distributions over the range of the
moderator replaces the classic JN-framework. If the moderator range is
given explicitly with fewer than eight unique values, distinct colors
are used to plot each The function \texttt{JNSienaBayes()} automatically
detects whether each required parameter has been estimated as a fixed
effect (\(\eta\); shared across groups) or as a random effect (\(\mu\);
...
\pandocbounded{\includegraphics[keepaspectratio]{int3ract_paper_files/figure-pdf/fig-bayes-2fake-1.pdf}}
...
\pandocbounded{\includegraphics[keepaspectratio]{int3ract_paper_files/figure-pdf/fig-bayes-2fake-2.pdf}}
...
formation when altX changes moderately, around -1 to 1 (yellow curves).
While reciprocity gets more positive with increasing values in the
change of altX, the posterior variance increases as well.}
...
\pandocbounded{\includegraphics[keepaspectratio]{int3ract_paper_files/figure-pdf/fig-bayes-3fake-1.pdf}}
...
{\small color\_mid} & Midpoint colour of diverging fill scale & {\small \cellcolor[HTML]{EBCC2A}{\textcolor[HTML]{000000}{`white'}}} \\ 
...
single model-agnostic function, \texttt{JNK\_int()}, the package invites
adoption beyond the two model classes for which dedicated wrappers
currently exist. Future development plans include:
...
and Girlich 2025; Müller and Wickham 2026).
